...
def urls_manager():
    ...
    for i in data_list:
        ...
        url = 'https://www.pixiv.net/ranking.php?mode=daily&content=illust&date=' + str(i)

        r = requests.get(url, headers=headers)

        rap = r"[a-zA-z]+://[^\s]*.jpg"
        r = re.compile(rap).findall(r.text)   # 先从网页上确认所有图片 缩小范围
        rap1 = "(6[0-9]{7})"
        num = re.compile(rap1).findall(str(r))  # 获取所有图片中 真正的所需要下载的作品ID

        id_list += num

    print ("已添加id   "+"共计"+str(len(id_list))+"个ID")
    for i in range(len(id_list)):               # 生成各个介绍页面
        url = "https://www.pixiv.net/member_illust.php?mode=medium&illust_id="+str(id_list[i])
        urls.append(url)
    print ("已添加url    "+"共计"+str(len(urls))+"个url")
    return urls,id_list
